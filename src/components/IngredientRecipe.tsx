...
import {
  deliveryValues,
  IngredientRecipeValues,
} from "../utils/FormInitialValue";
import { useFormik } from "formik";
import {
  DeliveryInputsSchema,
...
const IngredientRecipe = ({
  ...
  const {
    ...
    initialValues: ingredientsValues
      ? ingredientsValues
      : IngredientRecipeValues,
  ...
  const [isOthersNetQuantitySelected, setIsOthersNetQuantitySelected] =
    useState(false);
  const [isOthersWasteQuantitySelected, setIsOthersWasteQuantitySelected] =
    useState(false);

  // Update your handleChange function to track when "Others" is selected
  const handleDropdownChange = (e) => {
    const { name, value } = e.target;

    console.log("name= ", name, "value= ", value);

    if (name === "netQuantityUnit") {
      if (value === "Others") {
        setIsOthersNetQuantitySelected(true);
      } else if (quantitySizes.some((qs) => qs.value === value)) {
        setIsOthersNetQuantitySelected(false);
        handleChange(e);
      }
    }

    if (name === "wasteQuantityUnit") {
      if (value === "Others") {
        setIsOthersWasteQuantitySelected(true);
      } else if (quantitySizes.some((qs) => qs.value === value)) {
        setIsOthersWasteQuantitySelected(false);
        handleChange(e);
      }
    }
  };
      ...
      <div>
        ...
        <div className="w-full flex flex-row items-center justify-center gap-x-3">
          <Input
            ...
            error={
              errors.netQuantity && touched.netQuantity && errors.netQuantity
            }
          />

          {isOthersNetQuantitySelected ? (
            <Input
              type="text"
              ...
              error={
                errors.netQuantityUnit &&
                touched.netQuantityUnit &&
                errors.netQuantityUnit
              }
            />
          ) : (
        ...
        <div className="w-full flex flex-row items-center justify-center gap-x-3">
          <Input
            ...
            error={
              ...
              touched.wasteQuantity &&
              errors.wasteQuantity
            }
          />

          {isOthersWasteQuantitySelected ? (
            <Input
              type="text"
              newName="a quantity unit"
              placeholder=""
              ...
              error={
                errors.wasteQuantityUnit &&
                touched.wasteQuantityUnit &&
                errors.wasteQuantityUnit
              }
            />
          ) : (
